...
import com.example.im.contact.view.indexlib.indexBar.IndexBarDataHelperImpl
...
class WeChatIndexBar @JvmOverloads constructor(
    ...
    /**源数据 已经有序？ */
    private var isSourceDatesAlreadySorted = false
    ...
    private fun initData() {
        ...
        setOnLetterChangeListener(object : OnLetterChangeListener {
        ...
        when(event.action){
            ...
            MotionEvent.ACTION_MOVE -> {
                if (mPreSelect != mNewSelect && mNewSelect >= 0 && mNewSelect < mLetters!!.size) {
                    mSelect = mNewSelect
                    if (mListener != null) {
                        mListener?.onLetterChange(mLetters!![mSelect])
                    }
                }
                invalidate()
            }
    ...
    fun isSourceDatesAlreadySorted(): Boolean {
        return isSourceDatesAlreadySorted
    }
    /**
     * 源数据 是否已经有序
     *
    ...
    private fun getPosByTag(tag: String): Int {
        ...
        for (i in mSourceDates!!.indices) {
            if (tag == mSourceDates!![i].getBaseIndexTag()) {
                return i + getHeaderViewCount()
            }
        }
        return -1
    }
    private fun setOnLetterChangeListener(listener: OnLetterChangeListener) {
