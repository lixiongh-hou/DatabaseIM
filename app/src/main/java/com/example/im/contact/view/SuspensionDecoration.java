...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BOTTOM_LINE = Color.parseColor("#FFD9D9D9");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FF888888");
    /**title字体大小*/
    private int mTitleFontSize;
    private List<? extends ISuspensionInterface> mDates;
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mDates == null || mDates.isEmpty() || position > mDates.size() - 1 || position < 0 || !mDates.get(position).isShowSuspension()) {
                continue;//越界
            }
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    ...
                    if (null != mDates.get(position).getSuspensionTag() && !mDates.get(position).getSuspensionTag().equals(mDates.get(position - 1).getSuspensionTag())) {
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        ...
        mPaint.getTextBounds(mDates.get(position).getSuspensionTag(), 0, mDates.get(position).getSuspensionTag().length(), mBounds);
        c.drawText(mDates.get(position).getSuspensionTag(), child.getPaddingLeft() + 40, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        //pos为1，size为1，1>0? true
        if (mDates == null || mDates.isEmpty() || pos > mDates.size() - 1 || pos < 0 || !mDates.get(pos).isShowSuspension()) {
            return;//越界
        }

        String tag = mDates.get(pos).getSuspensionTag();
        ...
        if ((pos + 1) < mDates.size()) {//防止数组越界（一般情况不会出现）
            if (null != tag && !tag.equals(mDates.get(pos + 1).getSuspensionTag())) {//当前第一个可见的Item的tag，不等于其后一个item的tag，说明悬浮的View要切换了
...
//                    c.clipRect(parent.getPaddingLeft(), parent.getPaddingTop(), parent.getRight() - parent.getPaddingRight(), parent.getPaddingTop() + child.getHeight() + child.getTop());
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        position -= getHeaderViewCount();
        if (mDates == null || mDates.isEmpty() || position > mDates.size() - 1) {//pos为1，size为1，1>0? true
            return;//越界
        }
        //我记得Rv的item position在重置时可能为-1.保险点判断一下吧
        if (position > -1) {
            ISuspensionInterface titleCategoryInterface = mDates.get(position);
            //等于0肯定要有title的,
            ...
            if (titleCategoryInterface.isShowSuspension()) {
                if (position == 0) {
                    ...
                    if (null != titleCategoryInterface.getSuspensionTag() && !titleCategoryInterface.getSuspensionTag().equals(mDates.get(position - 1).getSuspensionTag())) {
                        //不为空 且跟前一个tag不一样了，说明是新的分类，也要title
                        outRect.set(0, mTitleHeight, 0, 0);
                    }
                }
