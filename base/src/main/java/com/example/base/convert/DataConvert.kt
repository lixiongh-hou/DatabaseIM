package com.example.base.convert

import com.example.base.error.ApiError
import com.example.base.base.BaseResponse
import com.example.base.base.BaseResponse1
import com.example.base.error.ServerException
...
object DataConvert {
    ...
    fun <T> BaseResponse<T>.convert(success: (T?) -> Unit) {
        if (this.success()) {
            ...
            when(this.errorCode){
                Status.LOGIN_ERROR -> throw ServerException(
                    ApiError(
                        this.errorCode,
     ...
     * 解析接口返回的数据和错误信息
     */
    fun <T> BaseResponse1<T>.convert1(success: (T?) -> Unit) {
        if (this.success()) {
            success(this.result)
        } else {
            when(this.code){
